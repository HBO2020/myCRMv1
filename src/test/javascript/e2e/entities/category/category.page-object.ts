...
export class CategoryUpdatePage {
  ...
  categoryDesciptionInput = element(by.id('field_categoryDesciption'));
  ...
  async setCategoryDesciptionInput(categoryDesciption: string): Promise<void> {
    await this.categoryDesciptionInput.sendKeys(categoryDesciption);
  }

  async getCategoryDesciptionInput(): Promise<string> {
    return await this.categoryDesciptionInput.getAttribute('value');
  }
