...
export class CommandeClientService {
  ...
  protected convertDateFromClient(commandeClient: ICommandeClient): ICommandeClient {
    return Object.assign({}, commandeClient, {
      cmdDateEffetCl: commandeClient.cmdDateEffetCl?.isValid() ? commandeClient.cmdDateEffetCl.format(DATE_FORMAT) : undefined,
      cmdDateLivraisonCl: commandeClient.cmdDateLivraisonCl?.isValid() ? commandeClient.cmdDateLivraisonCl.format(DATE_FORMAT) : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.cmdDateEffetCl = res.body.cmdDateEffetCl ? dayjs(res.body.cmdDateEffetCl) : undefined;
      res.body.cmdDateLivraisonCl = res.body.cmdDateLivraisonCl ? dayjs(res.body.cmdDateLivraisonCl) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commandeClient: ICommandeClient) => {
        commandeClient.cmdDateEffetCl = commandeClient.cmdDateEffetCl ? dayjs(commandeClient.cmdDateEffetCl) : undefined;
        commandeClient.cmdDateLivraisonCl = commandeClient.cmdDateLivraisonCl ? dayjs(commandeClient.cmdDateLivraisonCl) : undefined;
      });
    }
    return res;
  }
