...
export class CommandeFournisseurService {
  ...
  protected convertDateFromClient(commandeFournisseur: ICommandeFournisseur): ICommandeFournisseur {
    return Object.assign({}, commandeFournisseur, {
      cmdDateEffetFr: commandeFournisseur.cmdDateEffetFr?.isValid() ? commandeFournisseur.cmdDateEffetFr.format(DATE_FORMAT) : undefined,
      cmdDateLivraisonFr: commandeFournisseur.cmdDateLivraisonFr?.isValid()
        ? commandeFournisseur.cmdDateLivraisonFr.format(DATE_FORMAT)
        : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.cmdDateEffetFr = res.body.cmdDateEffetFr ? dayjs(res.body.cmdDateEffetFr) : undefined;
      res.body.cmdDateLivraisonFr = res.body.cmdDateLivraisonFr ? dayjs(res.body.cmdDateLivraisonFr) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commandeFournisseur: ICommandeFournisseur) => {
        commandeFournisseur.cmdDateEffetFr = commandeFournisseur.cmdDateEffetFr ? dayjs(commandeFournisseur.cmdDateEffetFr) : undefined;
        commandeFournisseur.cmdDateLivraisonFr = commandeFournisseur.cmdDateLivraisonFr
          ? dayjs(commandeFournisseur.cmdDateLivraisonFr)
          : undefined;
      });
    }
    return res;
